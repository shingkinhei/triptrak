...
const TabContent: FC<TabContentProps> = ({ trip, setTrip }) => {
  ...
  const handleShoppingItemCheck = (categoryId: string, itemId: string, checked: boolean) => {
    ...
    setTrip(currentTrip => currentTrip ? {...currentTrip, shoppingList: newList} : undefined);
    ...
    if (checkedItemName && checkedItemPrice) {
      ...
      if (!trip.transactions.some(t => t.id === newTransaction.id)) {
        setTrip(currentTrip => currentTrip ? {...currentTrip, transactions: [newTransaction, ...currentTrip.transactions] } : undefined);
      }
    }
  };
  ...
  const setTransactions = (updater: React.SetStateAction<Transaction[]>) => {
    setTrip(currentTrip => {
      ...
      const newTransactions = typeof updater === 'function' ? updater(currentTrip.transactions) : updater;
      return {...currentTrip, transactions: newTransactions};
    });
  };

  const setShoppingList = (updater: React.SetStateAction<ShoppingCategory[]>) => {
    setTrip(currentTrip => {
        if (!currentTrip) return undefined;
        const newShoppingList = typeof updater === 'function' ? updater(currentTrip.shoppingList) : updater;
        return {...currentTrip, shoppingList: newShoppingList};
      });
  }
  
  const componentProps = {
    planner: { itinerary: trip.itinerary, setItinerary: (updater: any) => setTrip(currentTrip => currentTrip ? {...currentTrip, itinerary: typeof updater === 'function' ? updater(currentTrip.itinerary) : updater } : undefined) },
    map: { locations: trip.itinerary.map(i => ({ lat: 35.6895, lng: 139.6917, name: i.title })) }, // Example, needs real coordinates
    expenses: { transactions: trip.transactions, setTransactions },
    shopping: { list: trip.shoppingList, setList: setShoppingList, onCheckChange: handleShoppingItemCheck },
  };
